...
- **Old-site topology (verified 2026-08-19):** `tpsventilatie.nl` = WordPress 7.1 + Oxygen + Contact Form 7 on LiteSpeed at `195.78.67.39` (`s161.cyber-folks.pl`), returns 200 with title "TPS Ventilatie". 9 pages, 0 posts. Its `robots.txt` advertises `/wp-sitemap.xml`, which serves a valid sitemap index today (200, re-probed 2026-09-21) and becomes a stale pointer at cutover — the real reason Phase 10 D-15 retires it. Nameservers moved from the old registrar's pair to dd24 on 2026-09-16 (Phase 9 D-26, record-for-record mirrored zone, 33/33 verified).
...
- **Access held for v1.1:** the repo, Vercel, Search Console, admin on the TPS Google Business Profile, and dd24 — the `tpsventilatie.nl` domain and its DNS (both zones). Nothing else on the old stack: WordPress admin, the cyberfolks hosting panel and the `info@tpsventilatie.nl` mailbox are all held by cyberfolks/Thomas (corrected 2026-09-21 by Phase 10 D-01 — this line previously claimed hosting access on the legacy stack, which we never held, and that is why MIG-01 became a public content mirror). Search Console is ours to run end-to-end this milestone.
