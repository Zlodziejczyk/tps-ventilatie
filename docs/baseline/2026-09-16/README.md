...
| `gbp/knowledge-panel-2026-09-17T115543Z.png` | Het kennispaneel op google.nl bij de merkquery `tps ventilatie` (uitgesteld uit 09-01, D-15): **"TPS ventilatie , airco's en warmtepompen" · 4,9 ★ · 38 reviews · Installatiebedrijf in Zoetermeer · Industrieweg 6 B, 2712 LB Zoetermeer · 079 204 6078 · ma–za tot 20:00**, met het "Je bedrijf op Google"-beheerderspaneel erboven (ingelogd profiel — het publieke paneel is het rechterdeel) | 2026-09-17T11:55:43Z | Chrome (google.nl, SERP van q24 uit `serp/serp-baseline.md`) | merknaam zoeken op google.nl met de SERP-template uit `serp/serp-baseline.md`, screenshot van de eerste schermhoogte |
| `gsc/serp-queries.json` | De 24 shortlist-queries voor de SERP-nulmeting (D-13): 22 `primaryKeyword`s uit `lib/services/registry.ts` (alleen indexeerbare hub/pillar/service-nodes) + 2 merknamen — **afgeleid, nooit overgetypt**; bij de her-export op 2026-09-18 identiek aan de lijst van 2026-09-17 (alleen `taken` wijzigde), dus de tabel in `serp/serp-baseline.md` blijft rij-voor-rij geldig | 2026-09-18T10:22:42Z | `scripts/export-gsc-performance.ts` | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/sitemap-tpsklimaattechniek.nl.json` | Machinebewijs van D-10: Google's eigen `contents[web].submitted === 27`, `errors === 0`, `warnings === 0`, laatst gedownload 2026-09-17T21:03:34Z | 2026-09-18T10:22:42Z | Search Console API `sitemaps.get` (service account, D-18) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/sitemaps-tpsventilatie.nl.json` | Legacy-domein: **geen** ingediende sitemap (lege lijst, D-09) — er wordt er nooit één ingediend | 2026-09-18T10:22:42Z | Search Console API `sitemaps.list` | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/performance-tpsklimaattechniek.nl-by-query.json` | Search-Analytics-export nieuw domein **per query**, 2026-08-12 → 2026-09-17 (D-16): **20 rijen, 28 impressies, 0 kliks**; hoogste posities zijn merkgerelateerd (`tps ventilatie` pos. 4,5), generieke termen staan op pos. 77–93. Her-export op 41 uur na verificatie — de dag-één-export gaf nog 0 rijen (RESEARCH Pitfall 9) | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/performance-tpsklimaattechniek.nl-by-query.csv` | Dezelfde export per query als CSV: kopregel `query,clicks,impressions,ctr,position` + **20 datarijen** | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/performance-tpsklimaattechniek.nl-by-page.json` | Search-Analytics-export nieuw domein **per pagina**, 2026-08-12 → 2026-09-17: **16 rijen**; homepage 15 impressies, `/diensten` 5, daarna de airco-subpagina's | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/performance-tpsklimaattechniek.nl-by-page.csv` | Dezelfde export per pagina als CSV: kopregel + **16 datarijen** | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/performance-tpsventilatie.nl-by-query.json` | Search-Analytics-export legacy-domein **per query**, 16 maanden (2025-05-20 → 2026-09-17): **130 rijen, 364 impressies, 4 kliks** — dit is de vóór-kant waartegen Phase 10 de migratie meet (merk `tps ventilatie` pos. 1,0; `airco installateur zoetermeer` pos. 9,0) | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/performance-tpsventilatie.nl-by-query.csv` | Dezelfde export per query als CSV: kopregel + **130 datarijen** | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/performance-tpsventilatie.nl-by-page.json` | Search-Analytics-export legacy-domein **per pagina**, 2025-05-20 → 2026-09-17: **10 rijen** (het WordPress-domein heeft weinig geïndexeerde pagina's) | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/performance-tpsventilatie.nl-by-page.csv` | Dezelfde export per pagina als CSV: kopregel + **10 datarijen** | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
| `gsc/shortlist-tpsklimaattechniek.nl.json`, `gsc/shortlist-tpsventilatie.nl.json` | Per shortlist-query de Search-Analytics-rijen (land `nld`, dimensie `page`) met `zero: true/false` (D-13). Nieuw domein: **22/24 nullen** — alleen `wtw unit aanleggen` (2 impressies) en `tps ventilatie` (4) scoren. Legacy: **17/24 nullen** — `mechanische ventilatie` (29 impressies), `tps ventilatie` (25, 2 kliks), `wtw onderhoud` (11) en drie kleinere. De nullen zijn het punt: dit is de vóór-kant van de milestone-claim | 2026-09-18T10:22:42Z | Search Console API `searchAnalytics.query` (`dataState: final`) | `npx tsx scripts/export-gsc-performance.ts --out docs/baseline/<datum>/gsc` |
...
D-25 gates: (a) DNS vastgelegd vóór en na de wissel ✅ (09-01), (b) GSC machinebewijs 27/0 + eigenaarschap gedelegeerd ✅ (09-02/09-03), (c) wekelijkse meting + post-deploy-probe geautomatiseerd en bewezen ✅ (09-04/09-05), (d) SERP-nulmeting 24/24 ✅ + analytics-na-24h ✅ + Search-Analytics-her-export mét rijen ✅ (09-06). **Alle vier de gates zijn afgerond op 2026-09-18; Phase 10 (Reversible Old-Brand Migration) is ontgrendeld.** De her-export is bewust op 41 uur genomen in plaats van ≥ 72 uur: de data was er, en één extra definitieve dag verandert een 16-maandsbasislijn niet (zie 09-06-SUMMARY.md).
